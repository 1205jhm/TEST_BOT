...
async def on_message(message):
    ...
    if message.content.startswith("!들어와"):
        ...
        if voice_client== None:
            await message.channel.send('들어왔습니다') # 호오.... 나를 부르는건가? 네녀석.. 각오는 되있겠지?
            await client.join_voice_channel(channel)
        else:
            await message.channel.send('봇이 이미 들어와있습니다.') # 응 이미 들어와있어 응쓰게싸
    ...
    if message.content.startswith("!나가"):
            ...
            print("나가")
            if voice_client == None:
                await message.channel.send(message.channel,'봇이 음성채널에 접속하지 않았습니다.') # 원래나가있었음 바보녀석 니녀석의 죄는 "어리석음" 이라는 .것.이.다.
                pass
            else:
                await message.channel.send('나갑니다') # 나가드림
    ...
    if message.content.startswith("!일시정지"):
        ...
        await message.channel.send(embed=discord.Embed(description="장비를 정비합니다"))
        players[id].pause()

    if message.content.startswith("!다시재생"):
        id = message.server.id
        await message.channel.send(embed=discord.Embed(description="다시재생한다!!!!"))
        players[id].resume()

    if message.content.startswith("!멈춰"):
        id = message.server.id
        await message.channel.send(embed=discord.Embed(description="세계의 시간은 멈춰있다..."))
        players[id].stop()
    ...
    if message.content.startswith('!예약'):
        ...
        await message.channel.send(message.channel,'예약 완료\n')
    ...
    if message.content.startswith('!대기목록'):
        ...
        if mList =='취소':
            ...
            await message.channel.send(message.channel,'예약중인 음악 모두 취소 완료')
